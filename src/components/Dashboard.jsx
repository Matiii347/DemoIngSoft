...
export default function Dashboard({ setView, alerts, setSelectedVehicleId }) {
  // Find critical alerts (error or warning types)
  const criticalAlerts = [
    { id: 1, title: 'Mantenimiento VM-402', desc: 'Frenos regenerativos', when: 'Hoy', urgency: 'high' },
    { id: 2, title: 'Revisión VM-115', desc: 'Sistema de enfriamiento', when: 'Mañana', urgency: 'warning' }
  ];
    ...
    <div className="flex flex-col gap-lg animate-fadeIn">
      ...
      <section className="grid grid-cols-2 gap-md">
        ...
        <div 
          ...
          className="col-span-2 bg-surface-container hover:bg-surface-container-high cursor-pointer rounded-xl p-md relative overflow-hidden flex flex-col justify-between min-h-[120px] border border-surface-variant/30 hover:border-primary/50 transition-all duration-300 group"
        >
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <div className="flex items-end gap-sm mt-xs">
                <span className="font-headline-xl text-headline-xl text-primary font-bold">54</span>
                <span className="font-body-md text-body-md text-on-surface-variant mb-1">/ 60</span>
              </div>
            </div>
          ...
          <div className="w-full bg-surface-container-highest rounded-full h-1.5 mt-sm relative z-10">
            <div className="bg-primary h-1.5 rounded-full transition-all duration-500" style={{ width: '90%' }}></div>
          </div>
        </div>
        ...
        <div className="bg-surface-container rounded-xl p-md flex flex-col justify-between min-h-[110px] border border-surface-variant/30">
          ...
          <p className="font-headline-lg-mobile text-headline-lg-mobile text-on-surface mt-sm font-bold">78%</p>
        </div>
        ...
        >
          <div className="flex justify-between items-start">
            <p className="font-label-md text-label-md text-on-tertiary-container">Alertas</p>
          ...
          <p className="font-headline-lg-mobile text-headline-lg-mobile text-on-tertiary-container mt-sm font-bold">03</p>
        </div>
      </section>
      ...
      <section className="bg-surface-container-high rounded-xl p-md flex flex-col gap-sm border border-tertiary/10">
        <div className="flex items-center gap-sm mb-xs">
          ...
          <h3 className="font-headline-sm text-headline-sm-mobile text-on-surface">Alertas Críticas</h3>
        </div>
          ...
          {criticalAlerts.map(alert => (
            <li key={alert.id} className="bg-surface-container rounded-lg p-sm flex items-center gap-md hover:bg-surface-container-high transition-colors">
              ...
              <div className="flex-1">
                <p className="font-label-md text-label-md text-on-surface">{alert.title}</p>
                <p className="font-body-sm text-body-sm text-on-surface-variant">{alert.desc}</p>
              </div>
              <span className={`font-label-md text-label-md ${alert.urgency === 'high' ? 'text-error' : 'text-tertiary'}`}>{alert.when}</span>
            </li>
          ))}
        </ul>
        <button 
      ...
      <section className="bg-surface-container rounded-xl p-md relative overflow-hidden border border-surface-variant/30">
        <div className="flex justify-between items-center mb-md relative z-10">
          <h3 className="font-headline-sm text-headline-sm-mobile text-on-surface">Ahorro de CO2</h3>
